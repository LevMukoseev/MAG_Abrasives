...
export const tabContent = [
  {
    ...
    content: (
      <div>
        ...
        <div className="mb-6">
          <img src="/images/valki.png" alt="Шлифовка прокатных валков" className="w-full max-w-2xl mx-auto rounded-lg shadow-md" />
        </div>
        
        <h4 className="text-xl font-bold text-gray-800 mb-4">Круги для рабочих валков горячей прокатки (ГП)</h4>
        <div className="flex flex-col md:flex-row gap-8 mb-4">
          <p className="text-gray-600">В качестве материалов для валков используются чугун с длительным сроком службы (ICDP), сталь с высоким содержанием хрома (High-Cr) и быстрорежущая сталь (HSS). Требования к поверхности валков ГП ниже, чем к валкам холодной прокатки.</p>
          <img src="/images/valki1.png" alt="Материалы для валков ГП" className="w-48 h-32 object-cover rounded-lg shadow-md" />
        </div>
        <div className="mb-4">
          <img src="/images/valki2.png" alt="Керамическое зерно NQN и Vortex" className="float-right ml-4 mb-4 w-48 h-32 object-cover rounded-lg shadow-md" />
          <p className="text-gray-600">В качестве материалов для валков используются чугун с длительным сроком службы (ICDP), сталь с высоким содержанием хрома (High-Cr) и быстрорежущая сталь (HSS). Требования к поверхности валков ГП ниже, чем к валкам холодной прокатки. Для более высокой скорости съёма и стойкости круга мы также добавляем зерно <span className="text-blue-600 font-bold">NQN</span> последнего поколения от 20 до 40% или Vortex (<span className="text-red-600 font-bold">АА</span>).</p>
        </div>

        <h4 className="text-xl font-bold text-gray-800 mb-4">Круги для рабочих валков холодной прокатки (ХП)</h4>
        <div className="flex flex-col md:flex-row gap-8 mb-4">
          <p className="text-gray-600">С поверхности валка снимается меньше металла, но при этом требуется высокое качество поверхности. Диаметр рабочего валка составляет приблизительно от 300 до 900 мм, а длина составляет 2500-5000 мм. Размер абразивного зерна колеблется от F36 до 120 по FEPA. В качестве материалов для валков в основном используются кованая и быстрорежущая сталь (HSS).</p>
          <img src="/images/valki3.png" alt="Рабочие валки холодной прокатки" className="w-48 h-32 object-cover rounded-lg shadow-md" />
        </div>
        <div className="mb-4">
          <img src="/images/valki4.png" alt="Шлифовальные круги для холодной прокатки" className="float-right ml-4 mb-4 w-48 h-32 object-cover rounded-lg shadow-md" />
          <p className="text-gray-600">При холодной прокатке, ввиду более высокой твердости валков, используются шлифовальные круги достаточно мягкие (градации твердости C…G) с высокой концентрацией керамического корунда <span className="text-blue-600 font-bold">NQN</span> (20-70%), либо <span className="text-red-600 font-bold">Vortex</span>, чтобы минимизировать время перешлифовки. Если Вам важен минимальный по времени оборот валков на вальцешлифовальном участке – выбирайте спецификации BEST. Размер зерна выбирайте по таблице или из опыта. С помощью режимов обработки можно получать Ra в достаточно широком диапазоне. Но в шлифовании есть правило – выбирать максимально крупное зерно, которое может обеспечивать требуемую чистоту поверхности. Так Вы быстрее снимите нужный припуск и меньше времени потратите на правку круга, а резание будет более свободное, меньше прижогов, выше стойкость круга и ниже себестоимость операции.</p>
        </div>
        ...
        <p className="text-gray-600 mb-4">На опорные валки обычно используются шлифовальные круги из обычного или белого оксида алюминия. Для более высокой скорости съёма и стойкости круга мы также добавляем зерно <span className="text-blue-600 font-bold">NQN</span> последнего поколения от 20 до 40% или Vortex (<span className="text-red-600 font-bold">АА</span>).</p>
